...
<body>
	...
	<?php
		...
		if( isset($_REQUEST["dni"]) && !empty($_REQUEST["dni"]) ) {
				...
				// Hacemos lo mismo que en la visualización de residencias, antes de crear la tabla comprobamos que hay datos
				$stmt->execute();
				if( $stmt->rowcount() <= 0 ) { // Función de PDO que me permite ver cuantas filas han sido afectadas
					die("No hay datos para el DNI introducido");
				}
